...
public class YKContainerManaEnchanter extends Container {

	protected ManaEnchanterInventory inventory; 
	
	/**
	 * コンストラクタ
	 */
	public YKContainerManaEnchanter(IInventory iinv, InventoryPlayer playerInv) {
		
		inventory = (ManaEnchanterInventory) iinv;
            ...
            for (int j = 0; j < invX; j++) {
            	...
            	this.addSlotToContainer(new YKSlotInventory(iinv, slotIndex, xPos, yPos) {
        			...
        			public boolean isItemValid(ItemStack stack) {
        				return stack.isEmpty() ? false : Items.ENCHANTED_BOOK == stack.getItem();
        		    }
            	});
            }
        }
		...
		this.addSlotToContainer(new YKSlotInventory(iinv, 16, 16, 77) {
			...
			public boolean isItemValid(ItemStack stack)
		    {
				return BotaniaHelper.recipesManaEnchanter.isCatalyst(stack);
		    }
		});
